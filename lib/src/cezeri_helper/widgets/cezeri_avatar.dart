...
class CezeriAvatar extends StatelessWidget {
  final String name;
  final double? radius;
  ...
  Widget build(BuildContext context) {
    ...
    if (imageBytes != null) {
      return _MyCircularImageBytes(radius: radius!, imageBytes: imageBytes!, onTap: onTap, onDoubleTap: onDoubleTap, onLongPress: onLongPress);
    }
    ...
    return InkWell(
      onTap: onTap ?? () => context.router.push(CezeriImageViewRoute(urls: [imageUrl!], initialIndex: 0)),
      onDoubleTap: onDoubleTap,
      onLongPress: onLongPress,
      child:
          kIsWeb
              ? Container(
                ...
                height: radius! * 2,
                decoration: BoxDecoration(shape: shape!, image: DecorationImage(image: NetworkImage(imageUrl!), fit: fit)),
              )
              : CachedNetworkImage(
                imageUrl: imageUrl!,
                imageBuilder:
