...
import '../../enums/src/payment_token_type.dart';
import '../../enums/src/sort_key_order.dart';
...
import '../../graphql_operations/storefront/mutations/checkout_complete_free.dart';
import '../../graphql_operations/storefront/mutations/checkout_complete_with_credit_card.dart';
...
import '../../graphql_operations/storefront/mutations/checkout_email_update.dart';
...
import '../../graphql_operations/storefront/mutations/checkout_line_item_add.dart';
import '../../graphql_operations/storefront/mutations/checkout_line_item_remove.dart';
import '../../graphql_operations/storefront/mutations/checkout_line_item_update.dart';
import '../../graphql_operations/storefront/mutations/checkout_line_items_replace.dart';
import '../../graphql_operations/storefront/mutations/checkout_shipping_address_update.dart';
import '../../graphql_operations/storefront/mutations/checkout_shipping_line_update.dart';
import '../../graphql_operations/storefront/mutations/complete_checkout_token_v3.dart';
import '../../graphql_operations/storefront/mutations/create_checkout.dart';
import '../../graphql_operations/storefront/queries/get_all_orders.dart';
import '../../graphql_operations/storefront/queries/get_checkout_info_requires_shipping.dart';
import '../../graphql_operations/storefront/queries/get_checkout_info_with_payment_id.dart';
import '../../graphql_operations/storefront/queries/get_checkout_info_with_payment_id_without_shipping_rates.dart';
import '../../graphql_operations/storefront/queries/get_checkout_information.dart';
import '../../graphql_operations/storefront/queries/get_checkout_without_shipping_rates.dart';
import '../../mixins/src/shopify_error.dart';
import '../../models/src/checkout/checkout.dart';
import '../../models/src/checkout/line_item/line_item.dart';
import '../../models/src/checkout/tokanized_checkout/tokanized_checkout.dart';
import '../../models/src/order/order.dart';
import '../../models/src/order/orders/orders.dart';
import '../../models/src/product/price_v_2/price_v_2.dart';
import '../../models/src/shopify_user/address/address.dart';
...
class ShopifyCheckout with ShopifyError {
  ...
  Future<Checkout> getCheckoutInfoQuery(String checkoutId, {bool getShippingInfo = true, bool withPaymentId = false}) async {
    final WatchQueryOptions optionsRequireShipping = WatchQueryOptions(
      document: gql(getCheckoutInfoAboutShipping),
      variables: {'id': checkoutId},
      fetchPolicy: ShopifyConfig.fetchPolicy,
    );
    QueryResult result = await _graphQLClient!.query(optionsRequireShipping);
    ...
    final QueryResult queryResult = (await _graphQLClient!.query(options));
    checkForError(queryResult);

    return Checkout.fromJson(queryResult.data!['node']);
  }
  ...
  Future<void> updateAttributes(String checkoutId, {bool? allowPartialAddresses, Map<String, String>? customAttributes, String? note}) async {
      ...
      variables: {
        ...
        'input': {
          ...
          if (customAttributes != null)
            'customAttributes': [
              for (var entry in customAttributes.entries) {'key': entry.key, 'value': entry.value},
            ],
    ...
    checkForError(result, key: 'checkoutAttributesUpdateV2', errorKey: 'checkoutUserErrors');
  }

  /// Returns all [Order] in a List of Orders.
  ///
  /// Returns a List of Orders from the Customer with the [customerAccessToken].
  Future<List<Order>?> getAllOrders(String customerAccessToken, {SortKeyOrder sortKey = SortKeyOrder.PROCESSED_AT, bool reverse = true}) async {
  ...
  Future<void> checkoutLineItemsReplace(String checkoutId, List<String> variantIdList) async {
    var checkoutLineItems = transformVariantIdListIntoListOfMaps(variantIdList);
    final MutationOptions options = MutationOptions(
      document: gql(replaceCheckoutItemsMutation),
      variables: {'checkoutId': checkoutId, 'checkoutLineItems': checkoutLineItems},
    );
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutLineItemsReplace', errorKey: 'userErrors');
  }

  /// Updates the shipping address on given [checkoutId]
  Future<Checkout> shippingAddressUpdate(String checkoutId, Address address) async {
    Map<String, dynamic> variables = address.toJson();
    variables['checkoutId'] = checkoutId;
    final MutationOptions options = MutationOptions(document: gql(checkoutShippingAddressUpdateMutation), variables: variables);
    final QueryResult result = await _graphQLClient!.mutate(options);
  ...
  Future<String?> completeCheckoutWithTokenizedPaymentV2({
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutCompleteWithTokenizedPaymentV2', errorKey: 'checkoutUserErrors');
  ...
  Future<Checkout> updateCheckoutEmail(String checkoutId, String email) async {
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutEmailUpdateV2', errorKey: 'checkoutUserErrors');
  ...
  Future<void> checkoutCustomerAssociate(String checkoutId, String customerAccessToken) async {
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutCustomerAssociateV2', errorKey: 'checkoutUserErrors');
  }
  ...
  Future<void> checkoutCustomerDisassociate(String checkoutId) async {
    final MutationOptions options = MutationOptions(document: gql(checkoutCustomerDisassociateMutation), variables: {'id': checkoutId});
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutCustomerDisassociateV2', errorKey: 'checkoutUserErrors');
  }

  /// Applies [discountCode] to the [Checkout] that [checkoutId] belongs to.
  Future<Checkout> checkoutDiscountCodeApply(String checkoutId, String discountCode) async {
    final MutationOptions options = MutationOptions(
      document: gql(checkoutDiscountCodeApplyV2Mutation),
      variables: {'checkoutId': checkoutId, 'discountCode': discountCode},
    );
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutDiscountCodeApplyV2', errorKey: 'checkoutUserErrors');
  ...
  Future<void> checkoutDiscountCodeRemove(String checkoutId) async {
    final MutationOptions options = MutationOptions(document: gql(checkoutDiscountCodeRemoveMutation), variables: {'checkoutId': checkoutId});
    QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutDiscountCodeRemove', errorKey: 'checkoutUserErrors');
  }
  ...
  Future<void> checkoutGiftCardAppend(String checkoutId, List<String> giftCardCodes) async {
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutGiftCardsAppend', errorKey: 'checkoutUserErrors');
  }

  /// creates a new [Checkout] with the given [lineItems], [shippingAddress] and [email].
  Future<Checkout> createCheckout({List<LineItem>? lineItems, Address? shippingAddress, String? email}) async {
    final MutationOptions options = MutationOptions(
              ...
              for (var lineItem in lineItems)
                {
                  ...
                  'customAttributes': lineItem.customAttributes.map((e) => {'key': e.key, 'value': e.value}).toList(),
                },
            ],
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutCreate', errorKey: 'checkoutUserErrors');
  ...
  Future<Checkout> addLineItemsToCheckout({required String checkoutId, required List<LineItem> lineItems}) async {
    final MutationOptions options = MutationOptions(
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'addLineItemsToCheckout', errorKey: 'checkoutUserErrors');
  ...
  Future<Checkout> updateLineItemsInCheckout({required String checkoutId, required List<LineItem> lineItems}) async {
    final MutationOptions options = MutationOptions(
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'updateLineItemsInCheckout', errorKey: 'checkoutUserErrors');
  ...
  Future<Checkout> removeLineItemsFromCheckout({required String checkoutId, required List<LineItem> lineItems}) async {
    final MutationOptions options = MutationOptions(
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'removeLineItemsFromCheckout', errorKey: 'checkoutUserErrors');
  ...
  Future<void> checkoutGiftCardRemove(String appliedGiftCardId, String checkoutId) async {
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutGiftCardRemoveV2', errorKey: 'checkoutUserErrors');
  }

  /// Returns [Checkout] that [checkoutId] belongs to, requires shipping.
  Future<Checkout> checkoutShippingLineUpdate(String checkoutId, String shippingRateHandle) async {
    final MutationOptions options = MutationOptions(
      document: gql(checkoutShippingLineUpdateMutation),
      variables: {'shippingRateHandle': shippingRateHandle, 'checkoutId': checkoutId},
    );
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutShippingLineUpdate', errorKey: 'checkoutUserErrors');
  ...
  Future<void> checkoutCompleteFree(String checkoutId) async {
    final MutationOptions options = MutationOptions(document: gql(checkoutCompleteFreeMutation), variables: {'checkoutId': checkoutId});
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutCompleteFree', errorKey: 'checkoutUserErrors');
  }
  ...
  Future<TokanizedCheckout> checkoutCompleteWithTokenizedPaymentV3(
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'checkoutCompleteWithTokenizedPaymentV3', errorKey: 'checkoutUserErrors');
    return TokanizedCheckout.fromJson(((result.data!['checkoutCompleteWithTokenizedPaymentV3'] ?? const {})['payment'] ?? const {}));
  }
}
