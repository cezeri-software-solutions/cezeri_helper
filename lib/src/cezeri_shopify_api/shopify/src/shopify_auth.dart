...
class ShopifyAuth with ShopifyError {
  ...
  Future<AccessTokenWithExpDate?> get accessTokenWithExpDate async {
    ...
    if (_currentCustomerAccessToken.containsKey(ShopifyConfig.storeUrl)) {
      accessTokenWithExpDate = AccessTokenWithExpDate.fromJson(_currentCustomerAccessToken[ShopifyConfig.storeUrl]!);
    } else {
      final prefs = await SharedPreferences.getInstance();
      if (prefs.containsKey(ShopifyConfig.storeUrl!)) {
        accessTokenWithExpDate = AccessTokenWithExpDate.fromJson(prefs.getString(ShopifyConfig.storeUrl!)!);
  ...
  ///
  /// The token is considered about to expire if the difference between the current time and the token's expiration time is less than 10 minutes.
  Future<bool> _isTokenNotExpiredButAboutToExpire(AccessTokenWithExpDate? data) async {
  ...
  Future<void> sendPasswordResetEmail({required String email}) async {
    final MutationOptions options = MutationOptions(document: gql(customerRecoverMutation), variables: {'email': email});
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'customerRecover', errorKey: 'customerUserErrors');
  }
  ...
  Future<ShopifyUser> signInWithEmailAndPassword({required String email, required String password}) async {
    ...
    final WatchQueryOptions getCustomer = WatchQueryOptions(
      document: gql(getCustomerQuery),
      variables: {'customerAccessToken': accessTokenWithExpDate.accessToken},
      fetchPolicy: ShopifyConfig.fetchPolicy,
    );
    final QueryResult result = await _graphQLClient!.query(getCustomer);
  ...
  Future<ShopifyUser> signInWithMultipassToken(final String multipassToken) async {
  ...
  Future<AccessTokenWithExpDate> _createAccessToken(String email, String password) async {
    final MutationOptions options = MutationOptions(document: gql(customerAccessTokenCreate), variables: {'email': email, 'password': password});
    final QueryResult result = await _graphQLClient!.mutate(options);
  ...
  Future<AccessTokenWithExpDate> _createAccessTokenWithMultipass(String multipassToken) async {
    final MutationOptions options = MutationOptions(
      document: gql(customerAccessTokenCreateWithMultipassMutation),
      variables: {'multipassToken': multipassToken},
    );
    final QueryResult result = await _graphQLClient!.mutate(options);
    return _extractAccessTokenWithExpDate(result.data, "customerAccessTokenCreateWithMultipass");
  }

  /// Helper method for extracting the customerAccessToken and Expiration date from the mutation.
  AccessTokenWithExpDate _extractAccessTokenWithExpDate(Map<String, dynamic>? mutationData, String mutation) {
    return AccessTokenWithExpDate.fromMap(mutationData?[mutation]['customerAccessToken'] ?? {});
  }
  ...
  Future<void> signOutCurrentUser() async {
    ...
    await _setShopifyUser(null, null);
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'customerAccessTokenDelete', errorKey: 'userErrors');
  }

  /// Renews customer access token
  Future<AccessTokenWithExpDate> _renewAccessToken(String customerAccessToken) async {
    final MutationOptions options = MutationOptions(
  ...
  Future<void> _setShopifyUser(AccessTokenWithExpDate? accessTokenWithExpDate, ShopifyUser? shopifyUser) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (accessTokenWithExpDate == null || accessTokenWithExpDate.accessToken == null || accessTokenWithExpDate.expiresAt == null) {
      _shopifyUser.remove(ShopifyConfig.storeUrl);
      _currentCustomerAccessToken.remove(ShopifyConfig.storeUrl);
      prefs.remove(_shopifyKey);
      prefs.remove(ShopifyConfig.storeUrl!);
    } else {
      _shopifyUser[ShopifyConfig.storeUrl] = shopifyUser;
      _currentCustomerAccessToken[ShopifyConfig.storeUrl] = accessTokenWithExpDate.toJson();
      prefs.setString(ShopifyConfig.storeUrl!, accessTokenWithExpDate.toJson());
    }
  }
  ...
  Future<void> deleteCustomer({required String userId}) async {
    ...
    final MutationOptions options = MutationOptions(document: gql(customerDeleteMutation), variables: {'id': userId});
    final QueryResult result = await _graphQLClientAdmin!.mutate(options);
    checkForError(result, key: 'customerDelete', errorKey: 'userErrors');
    await _setShopifyUser(null, null);
  }
}
