...
import '../../graphql_operations/storefront/queries/get_all_blogs.dart';
import '../../graphql_operations/storefront/queries/get_blog_by_handle.dart';
import '../../graphql_operations/storefront/queries/get_n_articles_sorted.dart';
...
class ShopifyBlog with ShopifyError {
  ...
  Future<List<Blog>?> getAllBlogs({SortKeyBlog sortKeyBlog = SortKeyBlog.HANDLE, bool reverseBlogs = false, bool reverseArticles = false}) async {
    final WatchQueryOptions options = WatchQueryOptions(
      document: gql(getAllBlogsQuery),
      variables: {'reverseBlogs': reverseBlogs, 'reverseArticles': reverseArticles, 'sortKey': sortKeyBlog.parseToString()},
  ...
  Future<Blog> getBlogByHandle(String handle, {SortKeyArticle sortKeyArticle = SortKeyArticle.RELEVANCE, bool reverse = false}) async {
  ...
  Future<List<Article>?> getXArticlesSorted(int articleAmount, {SortKeyArticle sortKeyArticle = SortKeyArticle.RELEVANCE}) async {
    final QueryOptions options = WatchQueryOptions(
      document: gql(getNArticlesSortedQuery),
      variables: {'x': articleAmount, 'sortKey': sortKeyArticle.parseToString()},
      fetchPolicy: ShopifyConfig.fetchPolicy,
    );
    final QueryResult result = await _graphQLClient!.query(options);
    checkForError(result);
