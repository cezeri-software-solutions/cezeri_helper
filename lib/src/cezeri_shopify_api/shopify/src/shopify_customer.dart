...
import '../../graphql_operations/storefront/mutations/customer_address_create.dart';
import '../../graphql_operations/storefront/mutations/customer_address_delete.dart';
import '../../graphql_operations/storefront/mutations/customer_address_update.dart';
import '../../graphql_operations/storefront/mutations/customer_default_address_update.dart';
import '../../graphql_operations/storefront/mutations/customer_update.dart';
...
class ShopifyCustomer with ShopifyError {
  ...
  Future<void> customerAddressUpdate({
    ...
    checkForError(result, key: 'customerAddressUpdate', errorKey: 'customerUserErrors');
  }
  ...
  Future<void> customerUpdate({
    ...
    final MutationOptions options = MutationOptions(document: gql(createValidMutationString(variableMap)), variables: variableMap);
    QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'customerUpdate', errorKey: 'customerUserErrors');
  }
  ...
  Future<Address> customerAddressCreate({
    ...
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'customerAddressCreate', errorKey: 'customerUserErrors');
  ...
  Future<void> customerAddressDelete({String? customerAccessToken, String? addressId}) async {
    final MutationOptions options = MutationOptions(
      document: gql(customerAddressDeleteMutation),
      variables: {'customerAccessToken': customerAccessToken, 'id': addressId},
    );
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'customerAddressDelete', errorKey: 'customerUserErrors');
  }

  /// updates the default adderess to the [addressId] provided from the customer to which [customerAccessToken] belongs to.
  Future<void> customerDefaultAddressUpdate({required String addressId, required String customerAccessToken}) async {
    final MutationOptions options = MutationOptions(
      document: gql(customerDefaultAddressUpdateMutation),
      variables: {'customerAccessToken': customerAccessToken, 'addressId': addressId},
    );
    final QueryResult result = await _graphQLClient!.mutate(options);
    checkForError(result, key: 'customerDefaultAddressUpdate', errorKey: 'customerUserErrors');
  }
}
