...
import '../../models.dart';
import 'product_media/product_media.dart';
...
class Product with _$Product {
  ...
  /// This is the price of the product variant with the title 'Default Title'
  /// It is useful to show the price of the product in the product list
  String formattedPriceWithLocale(String? locale) => productVariants.isEmpty ? '' : productVariants.first.price.formattedPriceWithLocale(locale);
  ...
  String compareAtPriceFormattedWithLocale(String? locale) =>
      productVariants.isEmpty
          ? ''
          : (productVariants.first.compareAtPrice == null ? '' : productVariants.first.compareAtPrice!.formattedPriceWithLocale(locale));
  ...
  double get compareAtPrice =>
      productVariants.isEmpty ? 0 : (productVariants.first.compareAtPrice == null ? 0 : productVariants.first.compareAtPrice!.amount);

  /// return the first product variant compareAtPrice formatted or empty string if the list is empty
  ///
  /// It is useful to show the compareAtPrice of the product in the product list
  String get compareAtPriceFormatted =>
      productVariants.isEmpty ? '' : (productVariants.first.compareAtPrice == null ? '' : productVariants.first.compareAtPrice!.formattedPrice);
  ...
  static List<ProductVariant> _getProductVariants(Map<String, dynamic> json) {
    try {
      if (json.containsKey('node')) {
        ...
        return ((json['node']?['variants']?['edges'] ?? []) as List).map((v) => ProductVariant.fromGraphJson(v ?? const {})).toList();
      } else {
        ...
        if (variants is Map) {
          ...
          return ((variants ?? []) as List).map((v) {
            ...
            jsonVariant['price'] = jsonVariant['price'] is PriceV2 ? jsonVariant['price'].toJson() : jsonVariant['price'];
            jsonVariant['unitPrice'] = jsonVariant['unitPrice'] is PriceV2 ? jsonVariant['unitPrice'].toJson() : jsonVariant['unitPrice'];
            jsonVariant['compareAtPrice'] =
                jsonVariant['compareAtPrice'] is PriceV2 ? jsonVariant['compareAtPrice'].toJson() : jsonVariant['compareAtPrice'];
            jsonVariant['image'] = jsonVariant['image'] is ShopifyImage ? jsonVariant['image'].toJson() : jsonVariant['image'];
            jsonVariant['unitPriceMeasurement'] =
                jsonVariant['unitPriceMeasurement'] is UnitPriceMeasurement
                    ...
                    : jsonVariant['unitPriceMeasurement'];
            jsonVariant['selectedOptions'] = jsonVariant['selectedOptions']?.map((e) => e is SelectedOption ? e.toJson() : e)?.toList();
  ...
  static List<AssociatedCollections> _getCollectionList(Map<String, dynamic> json) {
    try {
      if (json.containsKey('node')) {
        ...
        return ((json['node']?['collections']?['edges'] ?? []) as List).map((v) => AssociatedCollections.fromGraphJson(v ?? const {})).toList();
      } else {
  ...
  static List<ShopifyImage> _getImageList(Map<String, dynamic> json) {
    try {
      if (json.containsKey('node')) {
        ...
        return ((json['node']?['images']?['edges'] ?? []) as List).map((image) => ShopifyImage.fromJson(image['node'] ?? const {})).toList();
      } else {
        if (json['images'] == null) return [];
        if (json['images'] is Map) {
          if (json['images']?['edges'] == null) return [];
          return ((json['images']?['edges'] ?? []) as List).map((image) => ShopifyImage.fromJson(image['node'] ?? const {})).toList();
        } else {
  ...
  static List<ProductMedia> _getMediaList(Map<String, dynamic> json) {
    try {
      if (json.containsKey('node')) {
        ...
        return ((json['node']?['media']?['edges'] ?? []) as List).map((image) => ProductMedia.fromGraphJson(image ?? const {})).toList();
      } else {
        if (json['media'] == null) return [];
        if (json['media'] is Map) {
          if (json['media']?['edges'] == null) return [];
          return ((json['media']?['edges'] ?? []) as List).map((image) => ProductMedia.fromGraphJson(image ?? const {})).toList();
        } else {
  ...
  static List<Metafield> _getMetafieldList(Map<String, dynamic> json) {
    try {
      if (json.containsKey('node')) {
        ...
        //! CezeriChanges
        final metafields = ((json['metafields'] ?? []) as List).map((v) => Metafield.fromGraphJson(v ?? const {})).toList();
